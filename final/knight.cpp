/*
    ...
    These are the functions for the Knight piece.
*/
...
Knight::Knight(char (&t)[8][8], int (&p)[8][8], int (&d)[8][8], int row, int col, int pl, int id) : Piece::Piece(t, p, d, row, col, pl, id)
{    
    type = 'n';
    t[row][col] = type;
    #ifdef DEBUG
...
// Movement function
void Knight::move(char (&t)[8][8], int (&p)[8][8], int (&d)[8][8], int row, int col)
{
    bool moved = 0;
...
void Knight::calc_avail(char (&t)[8][8], int (&p)[8][8], int row, int col)
{
    // Get all relative moves allowed
    // Knight movements are independent of player
    rule[0][0] = 1, rule[0][1] = 2,
    rule[1][0] = 1, rule[1][1] = -2,
    rule[2][0] = 2, rule[2][1] = 1,
    rule[3][0] = 2, rule[3][1] = -1,
    rule[4][0] = -1, rule[4][1] = 2,
    rule[5][0] = -1, rule[5][1] = -2,
    rule[6][0] = -2, rule[6][1] = 1,
    rule[7][0] = -2, rule[7][1] = -1;
    #ifdef DEBUG
    ...
    // A knight is blocked if the square is occupied by same player
    for (int i = 0; i < MOVES; i++)
    {
        int t_row = avail[i][0];
        int t_col = avail[i][1];
